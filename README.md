...
**重要**: `.godot/` はコミットされないため、新規クローン直後は
グローバルスクリプトクラスキャッシュが存在しない。この状態でいきなり
`--headless --quit-after` 等を実行すると `class_name` 参照がパースエラーになる
（ADR-017参照）。**必ず一度だけ**ウォームアップを先に実行すること
（GUI エディタで「インポート」から開いた場合は不要、エディタ自身が
同等の処理を行う）:

```bash
# 初回のみ（.godot/ キャッシュを構築する）
...
`tools/ci_test.sh` はこのウォームアップを自動的に含むため、CIではそちらを使えばよい。

## サーバー起動（Phase 4 以降で実装予定）

上記と同じ理由で、本番デプロイでも `--server` の前に一度ウォームアップを
実行してから `.godot/` キャッシュごとデプロイするか、デプロイスクリプト内で
ウォームアップを先に走らせること。

```bash
godot --headless --editor --quit-after 60  # 初回のみ
